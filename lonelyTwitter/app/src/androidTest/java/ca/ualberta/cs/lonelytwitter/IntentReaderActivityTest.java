...
public class IntentReaderActivityTest extends ActivityInstrumentationTestCase2{
    ...
    public void testReverseText(){
        ...
        setActivityIntent(intent);
        IntentReaderActivity ira = (IntentReaderActivity) getActivity();
        assertEquals("The text should be reverse!","2 egassem", ira.getText());
    }

    public void testDefaultText(){
        Intent intent = new Intent();
        intent.putExtra(IntentReaderActivity.TEXT_TO_TRANSFORM_KEY, "default text");

        setActivityIntent(intent);
        IntentReaderActivity ira = (IntentReaderActivity) getActivity();

        TextView textView = (TextView) ira.findViewById(R.id.intentText);
        assertEquals("The text should be default message!", ira.getText(), "default text");
    }

    public void testAssertTextview(){
        Intent intent = new Intent();
